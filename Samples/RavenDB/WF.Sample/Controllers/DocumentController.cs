    ...
    public class DocumentController : Controller
    {
        ...
        public ActionResult Inbox(int page = 0)
        {
            ...
            return View("Index", new DocumentListModel()
            {
                Page = page,
                PageSize = pageSize,
                Docs = DocumentHelper.GetInbox(CurrentUserSettings.GetCurrentUser().ToString("N"), out count, page, pageSize),
                Count = count,
            });
        }
        ...
        public ActionResult Outbox(int page = 0)
        {
            ...
            return View("Index", new DocumentListModel()
            {
                Page = page,
                PageSize = pageSize,
                Docs = DocumentHelper.GetOutbox(CurrentUserSettings.GetCurrentUser().ToString("N"), out count, page, pageSize),
                Count = count,
            });
        }
        #endregion
        ...
        public ActionResult Edit(Guid? Id, Document model, string button)
        {
            ...
            using (var session = WF.Sample.Business.Workflow.WorkflowInit.Provider.Store.OpenSession())
            {
                ...
                {
                    target = session.Load<Document>(model.Id);
                    if (target == null)
                    {
