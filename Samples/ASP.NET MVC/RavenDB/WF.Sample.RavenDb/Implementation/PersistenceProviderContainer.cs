...
namespace WF.Sample.RavenDb.Implementation
{
    public class PersistenceProviderContainer : IPersistenceProviderContainer
    {
        ...
        private void GenereateDate(IDocumentSession session)
        {
            ...
            {
                if (session.Load<Business.Model.Role>(r.Id) == null)
                    session.Store(r);
            }

            foreach (var e in sd)
            {
                if (session.Load<Business.Model.StructDivision>(e.Id) == null)
                    session.Store(e);
            }
            ...
            if (session.Load<WorkflowScheme>("SimpleWF") == null)
            {
