...
namespace OptimaJet.Workflow.Redis
{
    public class ProcessInstanceTreeItem : IProcessInstanceTreeItem
    {
        ...
        public static List<IProcessInstanceTreeItem> Create(Guid rootProcessId, List<(Guid processId,Guid schemeId, Guid? parentProcessId, Guid rootProcessId)>
            instances, Dictionary<Guid,string> startingTransitions)
        {
            ...
            foreach ((Guid processId, Guid schemeId, Guid? parentProcessId, Guid rootProcessId) instance in instances)
            {
