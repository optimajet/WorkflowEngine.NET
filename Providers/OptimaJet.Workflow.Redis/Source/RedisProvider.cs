...
namespace OptimaJet.Workflow.Redis
{
    public class RedisProvider : IWorkflowProvider
    {
       ...
       public virtual async Task<XElement> GenerateAsync(string schemeCode, Guid schemeId, IDictionary<string, object> parameters)
        ...
        public string GetKeyAssignment(Guid assignmentId)
        {
            return $"{_providerNamespace}:assignment:{assignmentId:N}";
        }
        
        public string GetKeyAssignment(string assignmentId)
        {
            return $"{_providerNamespace}:assignment:{assignmentId}";
        }
        
        public string GetKeyAssignments()
        {
            return $"{_providerNamespace}:assignments";
        }
        
        public string GetKeyAssignmentCode(string assignmentCode)
        {
            return $"{_providerNamespace}:assignmentcode:{assignmentCode}";
        }
        
        public string GetKeyAssignmentProcess(Guid processId)
        {
            return $"{_providerNamespace}:assignmentprocessId:{processId:N}";
        }
        
        public string GetKeyAssignmentStatus(string status)
        {
            return $"{_providerNamespace}:assignmentstatus:{status}";
        }
        
        public string GetKeyAssignmentExecutor(string executor)
        {
            return $"{_providerNamespace}:assignmentexecutor:{executor}";
        }
        
        public string GetKeyAssignmentIsDeleted(bool isDeleted)
        {
            return $"{_providerNamespace}:assignmentisdeleted:{isDeleted}";
        }
        
        public string GetKeyAssignmentIsActive(bool isActive)
        {
            return $"{_providerNamespace}:assignmentisactive:{isActive}";
        }
        
        public string GetKeyAssignmentObserver(string observer)
        {
            return $"{_providerNamespace}:assignmentobserver:{observer}";
        }
...
#pragma warning restore 4014
        ...
        public async Task DeleteAssignmentAsync(Guid assignmentId) 
        {
            ...
            RedisValue assignmentValue = await db.HashGetAsync(GetKeyAssignments(), GetKeyAssignment(assignmentId)).ConfigureAwait(false);
            
            if (assignmentValue.HasValue)
            {
                var id = assignmentId.ToString("N");
                var a = JsonConvert.DeserializeObject<Assignment>(assignmentValue.ToString());

                if (a.Observers == null)
                {
                    a.Observers = new List<string>();
                }
                var batchTasks = new List<Task>();
                
                IBatch batch = db.CreateBatch();
                
                batchTasks.Add(batch.HashDeleteAsync(GetKeyAssignments(), GetKeyAssignment(assignmentId)));
                batchTasks.Add(batch.SetRemoveAsync(GetKeyAssignmentCode(a.AssignmentCode), id ));
                batchTasks.Add(batch.SetRemoveAsync(GetKeyAssignmentStatus(a.StatusState), id ));
                batchTasks.Add(batch.SetRemoveAsync(GetKeyAssignmentProcess(a.ProcessId), id ));
                batchTasks.Add(batch.SetRemoveAsync(GetKeyAssignmentExecutor(a.Executor), id ));
                batchTasks.Add(batch.SetRemoveAsync(GetKeyAssignmentIsDeleted(a.IsDeleted), id ));
                batchTasks.Add(batch.SetRemoveAsync(GetKeyAssignmentIsActive(a.IsActive), id ));
                foreach (var observer in a.Observers)
                {
                    batchTasks.Add(batch.SetRemoveAsync(GetKeyAssignmentObserver(observer), id ));
                }
                batch.Execute();
                await Task.WhenAll(batchTasks).ConfigureAwait(false);
            } 
        ...
        public async Task<List<Assignment>> GetAssignmentsAsync(AssignmentFilter filter, List<(string parameterName, SortDirection sortDirection)> orderParameters = null, Paging paging = null)
        {
            ...
            {
                var hashEntries = await db.HashGetAllAsync(GetKeyAssignments()).ConfigureAwait(false);
                foreach (var hashEntry in hashEntries)
                {
                    assignments.Add(JsonConvert.DeserializeObject<Assignment>(hashEntry.Value));
                }
            }
            else
            {
                var result = GetAssignmentByFilter(filter);
            
                var key = GetKeyAssignments();
                foreach (var assignmentId in result)
                {
                    var assignment = await db.HashGetAsync(key, GetKeyAssignment(assignmentId)).ConfigureAwait(false);

                    if (assignment.HasValue)
                    {
                        assignments.Add(JsonConvert.DeserializeObject<Assignment>(assignment.ToString()));
                    }
                }
            }

            assignments.Sort((x, y) => DateTime.Compare(x.DateCreation, y.DateCreation));
            ...
            return assignments.Skip(paging.SkipCount()).Take(paging.PageSize).ToList();
        }
        ...
        public async Task<int> GetAssignmentCountAsync(AssignmentFilter filter)
        {
            ...
            {
                IDatabase db = _connector.GetDatabase();
                var length =  await db.HashLengthAsync(GetKeyAssignments()).ConfigureAwait(false);
                return (int)length;
            }
            else
            {
        ...
        public async Task CreateAssignmentAsync(Guid processId, AssignmentCreationForm form)
        {
            ...
            {
                AssignmentId = form.Id ?? Guid.NewGuid(),
                ...
                Observers = form.Observers ?? new List<string>(),
            ...
            var id = a.AssignmentId.ToString("N");
            
            batchTasks.Add(batch.HashSetAsync(GetKeyAssignments(), GetKeyAssignment(a.AssignmentId), json));
            batchTasks.Add(batch.SetAddAsync(GetKeyAssignmentCode(a.AssignmentCode), id));
            batchTasks.Add(batch.SetAddAsync(GetKeyAssignmentStatus(a.StatusState), id ));
            batchTasks.Add(batch.SetAddAsync(GetKeyAssignmentProcess(a.ProcessId), id ));
            batchTasks.Add(batch.SetAddAsync(GetKeyAssignmentExecutor(a.Executor), id ));
            batchTasks.Add(batch.SetAddAsync(GetKeyAssignmentIsDeleted(a.IsDeleted), id ));
            batchTasks.Add(batch.SetAddAsync(GetKeyAssignmentIsActive(a.IsActive), id));
            foreach (var observer in a.Observers)
            {
                batchTasks.Add(batch.SetAddAsync(GetKeyAssignmentObserver(observer), id ));
            }
            batch.Execute();
            await Task.WhenAll(batchTasks).ConfigureAwait(false);
        }
        ...
        public async Task<Assignment> GetAssignmentAsync(Guid assignmentId)
        {
            ...
            var pair = await db.HashGetAsync(GetKeyAssignments(), GetKeyAssignment(assignmentId)).ConfigureAwait(false);

            if (pair.HasValue)
            {
                var assignment = JsonConvert.DeserializeObject<Assignment>(pair.ToString());
                return assignment;
            }
        ...
        public async Task UpdateAssignmentAsync(Assignment a)
        {
            ...
            RedisValue assignmentValue = await db.HashGetAsync(GetKeyAssignments(), GetKeyAssignment(a.AssignmentId)).ConfigureAwait(false);
            
            if (assignmentValue.HasValue)
            {
                var oldAssignment = JsonConvert.DeserializeObject<Assignment>(assignmentValue.ToString());

                if (oldAssignment.Observers == null)
                {
                    oldAssignment.Observers = new List<string>();
                }

                var batchTasksDel  = new List<Task>();
                IBatch batchDel = db.CreateBatch();
                
                batchTasksDel.Add(batchDel.HashDeleteAsync(GetKeyAssignments(), GetKeyAssignment(id)));
                batchTasksDel.Add(batchDel.SetRemoveAsync(GetKeyAssignmentCode(oldAssignment.AssignmentCode), id ));
                batchTasksDel.Add(batchDel.SetRemoveAsync(GetKeyAssignmentStatus(oldAssignment.StatusState), id ));
                batchTasksDel.Add(batchDel.SetRemoveAsync(GetKeyAssignmentProcess(oldAssignment.ProcessId), id ));
                batchTasksDel.Add(batchDel.SetRemoveAsync(GetKeyAssignmentExecutor(oldAssignment.Executor), id ));
                batchTasksDel.Add(batchDel.SetRemoveAsync(GetKeyAssignmentIsDeleted(oldAssignment.IsDeleted), id ));
                batchTasksDel.Add(batchDel.SetRemoveAsync(GetKeyAssignmentIsActive(oldAssignment.IsActive), id ));
                foreach (var observer in oldAssignment.Observers)
                {
                    batchTasksDel.Add(batchDel.SetRemoveAsync(GetKeyAssignmentObserver(observer), id ));
                }
                batchDel.Execute();
                await Task.WhenAll(batchTasksDel).ConfigureAwait(false);
                ...
                batchTasks.Add(batch.HashSetAsync(GetKeyAssignments(), GetKeyAssignment(id), json));
                batchTasks.Add(batch.SetAddAsync(GetKeyAssignmentCode(a.AssignmentCode), id)); 
                batchTasks.Add(batch.SetAddAsync(GetKeyAssignmentStatus(a.StatusState), id ));
                batchTasks.Add(batch.SetAddAsync(GetKeyAssignmentProcess(a.ProcessId), id ));
                batchTasks.Add(batch.SetAddAsync(GetKeyAssignmentExecutor(a.Executor), id ));
       ...
       public virtual async Task<List<ProcessHistoryItem>> GetProcessHistoryAsync(Guid processId, Paging paging = null)
        {
            ...
            if (paging == null)
            {
                history = await db.ListRangeAsync(GetKeyForProcessHistory(processId)).ConfigureAwait(false);
            }
            else
            {
                int skipIndex = paging.SkipCount();
                history = await db.ListRangeAsync(GetKeyForProcessHistory(processId), skipIndex, skipIndex + paging.PageSize - 1).ConfigureAwait(false);
            }
     
            return history.Select(hi => JsonConvert.DeserializeObject<WorkflowProcessTransitionHistory>(hi))
        ...
        {
            IDatabase db = _connector.GetDatabase();
            return Convert.ToInt32(await db.ListLengthAsync(GetKeyForProcessHistory(processId)).ConfigureAwait(false));
        }
...
#pragma warning restore 4014
        ...
        #region Internal
            ...
            private HashSet<string> GetAssignmentByFilter(AssignmentFilter filter)
        {
            IDatabase db = _connector.GetDatabase();
            ...
            {
                IEnumerable<RedisValue> res = new List<RedisValue>();
                switch (param.ExpressionType)
                {
                    case FilterExpressionType.Equal:
                        switch (param.Name)
                        {
                            case nameof(Assignment.Executor):
                            { 
                                res = db.SetMembers(GetKeyAssignmentExecutor((string) param.Value));
                                break;
                            }
                            case nameof(Assignment.ProcessId):
                            {
                                res = db.SetMembers(GetKeyAssignmentProcess((Guid)param.Value));
                                break;
                            }
                            case nameof(Assignment.AssignmentCode):
                            {
                                res = db.SetMembers(GetKeyAssignmentCode((string)param.Value));
                                break;
                            }
                            case nameof(Assignment.StatusState):
                            {
                                res = db.SetMembers(GetKeyAssignmentStatus((string)param.Value));
                                break;
                            }
                            case nameof(Assignment.Observers):
                            {
                                res = db.SetMembers(GetKeyAssignmentObserver((string)param.Value));
                                break;
                            }
                            case nameof(Assignment.IsDeleted):
                            {
                                res = db.SetMembers(GetKeyAssignmentIsDeleted((bool)param.Value));
                                break;
                            }
                            case nameof(Assignment.IsActive):
                            {
                                res = db.SetMembers(GetKeyAssignmentIsActive((bool)param.Value));
                                break;
                            }
                            default:
                                throw new Exception($"filter by {param.Name} is not supported");
                        }
                        break;
                    ...
                    case FilterExpressionType.In:
                        switch (param.Name)
                        {
                            case nameof(Assignment.StatusState):
                            {
                                var statuses = (IEnumerable<object>)param.Value;
                                foreach (var status in statuses)
                                {
                                     res = res.Concat(db.SetScan(GetKeyAssignmentStatus((string)status)));
                                }
                                break;
                            }
                            default:
                ...
                var assignmentString = res.Select(a => a.ToString());
                if (result == null)
                {
                    result = new HashSet<string>();
                    result.UnionWith(assignmentString);
                }
                else
                {
                    result.IntersectWith(assignmentString);
                }
            }
            return result;
        }
